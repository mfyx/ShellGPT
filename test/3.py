...
- [x] use `!` to launching external command-line programs
...
class FullShell(cmd.Cmd):
    ...
    def default(self, line):
        ...
        try:
            # 检查命令是否以 "!" 开头，如果是，则直接作为交互式命令运行
            if line.startswith("!"):
                interactive_command = line[1:].strip()
                subprocess.Popen(interactive_command, shell=True, stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr).wait()
            else:
                # 否则，尝试捕获输出并高亮显示
